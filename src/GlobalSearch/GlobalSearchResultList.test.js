...
describe('Global search results component', () => {
  it('should render results correctly', async () => {
    ...
    expect(tr.at(1).find('td a').at(OFFENDER_NAME_COLUMN).text()).toEqual('Quimby, Fred');
    expect(tr.at(1).find('td').at(NOMS_ID_COLUMN).text()).toEqual('A1234AC');
    expect(tr.at(1).find('td').at(LOCATION_COLUMN).text()).toEqual('LEEDS HMP');
    expect(tr.at(1).find('td').at(DOB_COLUMN).text()).toEqual('1977-10-15');

    expect(tr.at(2).find('td a').at(OFFENDER_NAME_COLUMN).text()).toEqual('Anderson, Arthur');
    expect(tr.at(2).find('td').at(NOMS_ID_COLUMN).text()).toEqual('A1234AA');
    expect(tr.at(2).find('td').at(LOCATION_COLUMN).text()).toEqual('Moorland HMP');
    expect(tr.at(2).find('td').at(DOB_COLUMN).text()).toEqual('1976-09-15');
  });
