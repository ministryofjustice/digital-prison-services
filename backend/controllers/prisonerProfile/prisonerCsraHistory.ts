  ...
  async (req, res) => {
    ...
    try {
      ...
      return res.render('prisonerProfile/prisonerCsraHistory.njk', {
        ...
        locationOptions: agenciesWithDescriptions.map((agency) => ({
        ...
        rows: sortedRelavantResults.map((assessment) => [
          ...
          {
            text:
              agenciesWithDescriptions.find((agency) => agency.agencyId === assessment.assessmentAgencyId)
                ?.description || 'Not entered',
          },
          { text: assessment.assessmentComment || 'Not entered' },
          {
            html: `<a class="govuk-link" href="/prisoner/${offenderNo}/csra-review?assessmentSeq=${assessment.assessmentSeq}&bookingId=${assessment.bookingId}">View details</a>`,
          },
        ]),
      })
    } catch (error) {
