...
module.exports = ({ prisonApi, personService, allocationManagerApi }) => async (req, res) => {
  ...
  const contactForEachAddress =
    ...
    (await Promise.all(
      activeOfficialContacts.sort((left, right) => left.firstName.localeCompare(right.firstName)).map(async contact => {
