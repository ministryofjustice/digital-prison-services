...
export default ({ oauthApi, prisonApi, whereaboutsApi, nonAssociationsApi }) =>
  async (req, res) => {
    ...
    try {
      ...
      const receptions = await prisonApi.getReceptionsWithCapacity(res.locals, prisonerDetails.agencyId)

      const residentialLevelNonAssociations = await getResidentialLevelNonAssociations(res, {
        prisonApi,
        nonAssociations,
        cellId: hasLength(receptions) && receptions[0].id,
        agencyId: prisonerDetails.agencyId,
        location,
      })

      const selectedReceptions = receptions.filter((cell) => {
      ...
      return res.render('cellMove/selectReception.njk', {
        formValues: {
          location,
          subLocation,
          cellType,
