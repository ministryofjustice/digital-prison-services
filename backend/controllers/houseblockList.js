...
const getHouseblockListFactory = (elite2Api, whereaboutsApi, config) => {
  ...
  const getHouseblockList = async (context, agencyId, groupName, date, timeSlot, stayingOnWing) => {
    ...
    const offendersStayingOnWing = distinct(
      data.filter(
        offender =>
          offender.locationCode === 'WOW' ||
          offender.locationCode === 'STAYONWING' ||
          offender.locationCode === 'UNEMPLOYED' ||
          offender.locationCode === 'RETIRED'
      )
    )
