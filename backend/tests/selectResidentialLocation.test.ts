...
describe('Select residential location controller', () => {
  ...
  beforeEach(() => {
    ...
    locationsInsidePrisonApi.searchGroups = jest.fn().mockResolvedValue([
  ...
  describe('index', () => {
    it('should makes the expected API calls', async () => {
      ...
      expect(locationsInsidePrisonApi.searchGroups).toHaveBeenCalledWith(systemContext, 'MDI')
    })
    ...
    describe('when there is an error retrieving data', () => {
      it('should set correct redirect url and throw error', async () => {
        ...
        locationsInsidePrisonApi.searchGroups = jest.fn().mockRejectedValue(error)
