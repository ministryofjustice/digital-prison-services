  ...
  context('Basic page functionality', () => {
      ...
      cy.task('stubSchedule', {
        ...
        thisWeeksSchedule: [
          ...
          },
          {
            ...
            eventDate: moment(today).add(1, 'day').format('YYYY-MM-DD'),
          ...
          },
          {
            ...
            eventDate: moment(today).add(2, 'days').format('YYYY-MM-DD'),
            startTime: moment(today)
              .add(2, 'days')
              .set('hour', 20)
              .set('minute', 0)
    ...
    context('when viewing current weeks schedule', () => {
      ...
      it('should show the correct dates in the title and link', () => {
        ...
        cy.get('[data-test="schedule-dates"]').should(
          'contain.text',
          `${moment().format(longDateFormat)} to ${moment().add(6, 'days').format(longDateFormat)}`
        )
        cy.get('[data-test="schedule-select-week"]').should(
          'contain.text',
          `View 7 days from ${moment().add(1, 'week').format('D MMMM YYYY')}`
        )
      })

      it('should display the correct days', () => {
        cy.get('[data-test="schedule-day"]').then(($days) => {
          cy.get($days).its('length').should('eq', 7)
          cy.get($days).each(($el, index) => {
            if (index === 0) {
              cy.get($el).should('contain.text', moment().format(longDateFormat))
            } else {
              cy.get($el).should('contain.text', moment().add(index, 'days').format(longDateFormat))
            }
          })
        })
      })
    ...
    context('when viewing next weeks schedule', () => {
      ...
      it('should show the correct dates in the title and link', () => {
        ...
        cy.get('[data-test="schedule-dates"]').should(
          'contain.text',
          `${moment().add(1, 'week').format(longDateFormat)} to ${moment().add(13, 'days').format(longDateFormat)}`
        )
      ...
      it('should display the correct days', () => {
        cy.get('[data-test="schedule-day"]').then(($days) => {
          ...
          cy.get($days).each(($el, index) => {
            if (index === 0) {
              cy.get($el).should('contain.text', moment().add(1, 'week').format(longDateFormat))
            } else {
              cy.get($el).should(
                'contain.text',
                moment()
